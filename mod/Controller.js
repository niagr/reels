...
function Controller() {
    ...
    window.tmdb = new TMDb("f6bfd6dfde719ce3a4c710d7258692cf");
...
//         load genres list
	...
	this.load_new_movies_from_dir = function(dir_entry) {

		load_files(dir_entry, onLoadFiles);
		...
		function onLoadFiles(video_file_list) {
			...
			process_new_movies(new_movie_list);
	...
	var load_files = function(dir, cb) {
...
//			say("Started reading directory: " + dir.name);
		...
		function select_new_video_files(file_list) {
			...
			file_list.forEach(function(file) {
				...
				if ((file.basename.slice(-3) == "avi") || (file.basename.slice(-3) == "mkv") || (file.basename.slice(-3) == "mp4")) {

					// iterate through movie list to check for duplicates with filename
					var dupe = false;
					that.movie_list.forEach(function(movie) {

						if (file.basename == movie.video_file.basename) {
							dupe = true;
						}
	...
	var process_new_movies = function(new_movie_list) {
		...
		new_movie_list.forEach(function(movie, index, list) {
			...
			if (movie.infer_title_and_year()) {
                ...
                say(" Could not infer the name of file: " + movie.video_file.name);
        ...
        function save_info(movie) {
            ...
            var entry = {
//                video_file_ID: chrome.fileSystem.retainEntry(movie.video_file),
    ...
    this.load_stored_movies = function() {
        ...
        console.log("aaaaaa");
            ...
            $.each(stored, function(key, item) {
                ...
                function onRestoreEntry(entry) {
                    ...
                    that.app_data_dir.getFile(movie.movie_info.id.toString() + ".jpg", null, function(ent) {
